...
# log setting
logging.basicConfig(filename='Save_BusinessDismiss.log', level=logging.DEBUG,
...
# 取得公司資料檔案
path = "C:/Users/james/OneDrive/桌面/ncku/職業趨勢/商業解散"
        ...
        for file in allFileList:
            ...
            with open(os.path.join(path, file), newline='', encoding="utf-8") as csvfile:
                ...
                for csv_row in spamreader:
                    ...
                    print(tax_id, name, date, dismiss_month)
                    ...
                    # 匯入資料
                    if insertCount == 0:
                        sql = 'INSERT INTO `company_dismiss`(`tax_id`, `name`, `date`, `month`) VALUES\n'
                        isLast = False

                    if insertCount < 100:
                        if tax_id.isdigit():
                            sql += f'("{tax_id}", "{name}", {date}, {dismiss_month}),\n'
                        else:
                            sql += f'({tax_id}, {name}, {date}, {dismiss_month}),\n'
                        insertCount += 1
                    else:
            ...
            if keepGo == 't':
                f = open('匯入完成月份(商業).txt', 'a', encoding="utf-8")
                f.writelines(file + '\n')
                f.close()
            elif keepGo == 'f':
                break
