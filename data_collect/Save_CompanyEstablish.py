...
# log setting
logging.basicConfig(filename='Save_CompanyEstablish.log', level=logging.DEBUG,
...
# 取得公司資料檔案
path = "C:/Users/james/OneDrive/桌面/ncku/職業趨勢/公司設立"
        ...
        for file in allFileList:
            ...
            with open(os.path.join(path, file), newline='', encoding="utf-8") as csvfile:
                ...
                for csv_row in spamreader:
                    ...
                    if csv_row[-1][:3] and csv_row[-1][:3].isdigit():
                        # date = yyyy:mm:dd
                        date = '"' + str(int(csv_row[-1][:3]) + 1911) + '-' + csv_row[-1][3:5] + '-' + csv_row[-1][5:] + '"'
                    elif csv_row[-1][1:4] and csv_row[-1][1:4].isdigit():
                    ...
                    # 無tax_id未來處理
                    try:
                        if csv_row[-2].isdigit():
                            int(tax_id)
                        else:
                            int(tax_id[1:-1])
                    except Exception as e_no_tax_id:
                        if csv_row[-2].isdigit():
                            sql_no_tax_id = f'INSERT INTO `com_no_tax_id`(' \
                                            f'`tax_id`, `name`, `address`, `head`, `capital_amount`, `establish_date`' \
                                            f') VALUES (' \
                                            f'"{tax_id}", "{name}", "{csv_row[3]}", "{csv_row[4]}", ' \
                                            f'"{csv_row[5]}", "{csv_row[6]}");'
                        else:
            ...
            keepGo = 't'
            if keepGo == 't':
                f = open('匯入完成月份(公司設立).txt', 'a', encoding="utf-8")
                f.writelines(file + '\n')
                f.close()
            elif keepGo == 'f':
                break
...
"""
CREATE TABLE `occupational_trends`.`company_establish` (
...
`date` DATE NOT NULL COMMENT '設立日期' ,
`month` INT(6) NOT NULL COMMENT '設立月份(yyyymm)' ,
