...
# 取得系所檔案
# "C:\Users\james\OneDrive\桌面\ncku\職業趨勢\系所\112_ulistdepartmentlist_college.csv"
path = "C:/Users/james/OneDrive/桌面/ncku/職業趨勢/系所/112_ulistdepartmentlist_college.csv"
    ...
    with connect_db.cursor() as cursor:
        with open(path, newline='', encoding="utf-8") as csvfile:
            ...
            for row in list(spamreader)[3:]:
                ...
                print(d_code, name, college)

                # 匯入資料
                if insertCount == 0:
                    sql = 'INSERT INTO `department`(`d_code`, `name`, `college`) VALUES\n'
                    isLast = False

                if insertCount < 100:
                    sql += f'("{d_code}", "{name}", "{college}"),\n'
                    insertCount += 1
                else:
                    sql += f'("{d_code}", "{name}", "{college}")\n'
